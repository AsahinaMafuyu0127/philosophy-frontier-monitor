...
缓存可能耗时较长并占用较多 F: 本地空间；stderr 会输出页数和累计记录。当前缓存以完整窗口为
提交单位，尚不把未完成分页的 resumptionToken 保存为断点；中断的缺口需要重新收割。完全没有
feed 时间和年份且未命中成功 OAI 窗口的非开放记录，不再通过付费墙或逐篇 PhilPapers 抓取扩展
范围；OAI 失败时仍恢复宽候选，不能把失败当作排除证据。
