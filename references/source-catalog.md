...
- 作为完整增量候选入口：`stable`（仍需与分类 feed 及独立书目证据求交）；
...
### 2026-09-08 跨运行缓存与秒级窗口实测

PhilArchive 的实际 header datestamp 精确到秒。适配器因此把初始 `from` 向前重叠一秒，并把包含式
`until` 设为半开窗口结束前的最后一个完整秒；后续页仍只提交不透明 `resumptionToken`。默认的
私人 `oai-cache.sqlite3` 保存最小记录事件和完整成功的覆盖区间，相同窗口不再访问网络，滚动窗口
只补齐缺口。缓存保留删除事件，但不落盘题名、作者、摘要、正文或完整 XML。

真实固定窗口依次验证了冷启动、关闭进程后热命中和尾部增量刷新：一小时冷窗口取得 43 条；同一
窗口热命中取得相同 43 条且 OAI 网络请求为 0；向后扩展半小时只从网络取得 6 条，合并后为 49 条。
随后用不公开具体研究方向的多分类私人配置运行完整 `pull-now`。上游在 2026-09-07 出现一次密集
批量元数据更新，首次补齐读取约 13.8 万条记录并完成 140 个 OAI 逻辑请求；紧接着的完整复跑只
刷新尾部 1 条、使用 1 个 OAI 请求。两次都没有推进周报状态。

这一异常量说明 OAI datestamp 洪峰可以来自整批元数据维护，不能解释成同期发表量。首次大窗口
缓存可能耗时较长并占用较多 F: 本地空间；stderr 会输出页数和累计记录。`v0.3.0` 起，每个成功页
把最小记录与下一枚不透明 `resumptionToken` 同事务暂存；中断后从最近检查点恢复。完整序列才会
原子提升为正式缓存与覆盖区间；过期或被上游拒绝的令牌只使对应缺口从原窗口重启。完全没有 feed
时间和年份且未命中成功 OAI 窗口的非开放记录，不再通过付费墙或逐篇 PhilPapers 抓取扩展范围；
OAI 失败时仍恢复宽候选，不能把失败当作排除证据。

固定五小时真实窗口的进程级恢复试验在第二页后强制退出：当时正式事件和覆盖均为 0，暂存 1998
条并保留下一页检查点；新进程直接从第三页继续，最终完成 118 页、116971 条精确窗口记录。随后
同窗口热运行发出 0 个 OAI 请求。生产滚动窗口又验证先完成旧中断会话、再补齐新尾部；紧接着的
第二次即时拉取仅收割新增尾部，机器延期由 7 条归零，周报状态散列在两次运行前后不变。
...
- 独立聚合诊断显示，把批量题名过滤从宽搜索改为 `title.search.exact`，严格候选匹配由 72 提高到
  91，但仍约有 247 条需要逐篇退回。这是有益的小幅优化，不足以消除当时的冷启动成本；后续版本
  通过有界轮转、跨运行缓存和中断恢复解决资源与可恢复性问题，而没有把外部来源未覆盖伪装成完成。
...
### 2026-09-09 首次拉取预算与无日期集合

- 默认逐篇远程候选预算由 50 提高到 300；在 1000 个总候选上限下，DOI、题名和逐篇路径的保守
  计划上界相应为 470 个 OpenAlex 和 300 个 Crossref 逻辑请求。程序在联网前明确提醒首次运行可能
  更久；实际请求量仍取决于缓存、DOI 覆盖、题名批量结果和来源熔断。
- 已命中完整 OAI 窗口、但同时缺少 feed 时间、feed／OAI 年份、DOI 和明确早期稿本类型的库存记录
  不再反复进入题名或逐篇查询。默认私人书目缓存只保存其 PhilPapers 来源 ID 的命名空间散列与
  首次／最近观察时间，普通报告只提示集合数量。一旦来源后来补入任一信号，记录自动回到核验路径。
- 这一集合是请求规划边界，不是书目事实：它不证明旧作或不相关，也不能进入周报通知状态。代价是
  一个真正新出、却完全不带上述信号的手稿可能在普通即时报告中遗漏；该风险必须随集合数量披露。
- 三个公开示例分类的真实七日冷运行在 574 条窗口候选中首次建立 276 条集合成员；集合外 136 条
  逐篇候选全部执行证据判断，其中 128 条实际访问远程服务；当时名为 `machine_deferred`、现名为
  `remote_verification_not_reached` 的统计为 0。紧接着的热运行报告 276 条全部复用、0 条新入集合，
  题名批量 API 值为 0，本次未轮到远程核验仍为 0；OAI 尾部刷新从
  首轮 139 个逻辑请求降为 1 个。两轮各有一次 OpenAlex 失败并触发本轮熔断，所以仍分别保留 69／61
  条自动重试，而没有把来源失败混写成预算积压。周报状态数据库未被打开或修改。
- 修正查询级永久 4xx 不应打开全来源熔断后，第三次热运行继续处理后续 OpenAlex 候选，直至免费
  额度剩余 8 且出现超出本轮等待预算的来源级失败；匹配由上一轮的 12 条增至 34 条，自动重试由
  61 条降至 29 条，本次未轮到远程核验仍为 0。该结果说明提高本地预算和修正熔断能够消除可控的
  当次未处理量，但不能
  把提供方真实的日额度／等待要求伪装成“已全部核验”。
