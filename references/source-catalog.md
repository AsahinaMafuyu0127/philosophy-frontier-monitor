...
- 2026-09-06 真实 PhilPapers 八分类即时测试把 2039 条当前记录按书目年份缩小为 345 条候选；
  DOI 直接链接只覆盖 7 条，因此必须保留批量题目路径。使用免费 key 并经用户明确授权把单次退回
  上限提高到 300 后，完整七日运行成功：79 条批量确认、266 条逐篇退回，最终形成 122 个唯一
...
- 2026-09-06 后续实现不再把 OpenAlex 当作新论文的必要登记机关：查到更早的等价作品时用于排除
  ...
  当前提醒证据形成 `confirmed_source_arrival`。该语义仍待一次新的真实八分类即时运行验证。
...
逐篇查无记录保存 15 分钟；题名批量只缓存有返回的单题名候选并在 1 小时后过期，空题名结果不
缓存。请求失败与语义复核不缓存；缓存不能压制即时报告与周报之间允许发生的重复推送。
